...
\bigbreak
{\large \textbf{Цели и задачи работы}}
\bigbreak
...
\\
Для удобства и простоты понимания нижеприведенных задач, под аббревиатурой ПО будем понимать "программное обеспечение необходимое для функционирования рекламной сети".
Для разработки ПО согласно каскадной модели разработки необходимо выполнить следующие задачи (каскадный процесс разработки ПО):
\begin{itemize}
...
\begin{enumerate}
	...
	\item Возможность гибкого контроля и индивидуальной конфигурации рекламных материалов в мобильных приложениях компании
...
\begin{lstlisting}[float=!h,caption={Пример исходного кода на Java},label={lst1}]
	constructor(
        public readonly trackRepository: TrackRepository, // TODO make private
        public readonly impressionRepository: ImpressionRepository, // TODO make private
        public readonly clickRepository: ClickRepository // TODO make private
    ) {}

    public async createTrack(adId: string, unitId: string, strategyId: number = 0): Promise<Track> {
        const track = new Track(adId, unitId, strategyId)
