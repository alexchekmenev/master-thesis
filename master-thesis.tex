...
\begin{lstlisting}[style=ES6,float=!h,caption={Пример исходного кода на Java},label={lst1}]
export class StrategyStateResolver {
    constructor(
        private readonly strategyStateRepository: StrategyStateRepository,
        private readonly sessionRepository: SessionRepository
    ) {
    }

    public async resolveKey(sessionKey: Session.Key, strategyId: StrategyId, threadId: string): Promise<StrategyState.Key> {
        let session = await this.sessionRepository.get(sessionKey)
        if (!session) {
            const lastStateKey =
                await this.strategyStateRepository.getThreadLastStateKey(strategyId, threadId)
            if (!lastStateKey) {
                throw new Error(`There are no states for strategy=${strategyId} and thread=${threadId}`)
            }
            session = new Session(sessionKey, new Session.Value(lastStateKey))
            await this.sessionRepository.save(session)
        }
        return session.value.strategyStateKey
    }
}
\end{lstlisting}
